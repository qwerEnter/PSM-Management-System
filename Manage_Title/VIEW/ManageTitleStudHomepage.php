...
if(isset($_POST['save']))
{
    ...
    {
        $search = $_POST['search'];
        $stmt = $conn->prepare("select * from lecturers where Lect_Name like '%$search%' or name like '%$search%'");
        $stmt->execute();
    ...
    <div class="table-responsive">          
      <table class="table">
		...
		<?php
			...
			$records = mysqli_query($db,"SELECT * FROM title"); // fetch data from database
