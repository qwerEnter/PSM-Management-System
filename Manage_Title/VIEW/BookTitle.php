...
<center>
    ...
    $sql = "SELECT * FROM title t JOIN student s ON s.Stud_ID = t.Stud_ID WHERE s.Stud_ID = '$currentUser'";

    $gotResult = mysqli_query($conn, $sql);

    if ($gotResult) {
        if (mysqli_num_rows($gotResult) > 0) {
            $data = mysqli_fetch_assoc($gotResult);
        } else {
...
?>
    ...
    <div class="form-group">
        ...
        <input type="id" class="form-control" id="id" placeholder="Enter student ID" name="Stud_ID" value="<?= isset($data['Stud_ID']) ? $data['Stud_ID'] : '' ?>">
    </div><br>
    <div class="form-group">
        <label for="Stud_Name">Student Name</label>
        <input type="name" class="form-control" id="name" placeholder="Enter student name" name="Stud_Name" value="<?= isset($data['Stud_Name']) ? $data['Stud_Name'] : '' ?>">
    </div><br>
    <div class="form-group">
        <label for="Title_Name">Title Name</label>
        <input type="name" class="form-control" id="name" placeholder="Enter title" name="Title_Name" value="<?= isset($data['Title_Name']) ? $data['Title_Name'] : '' ?>">
    </div>
    <br>
